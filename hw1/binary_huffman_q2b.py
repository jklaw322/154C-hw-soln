...
def tunstall(src_pmf,num_phrases):
    '''src_pmf,num_phrases MUST be numpy arrays'''
    pmf = sorted(src_pmf,reverse=True)
    map(lambda x: x*pmf[0],pmf)
...
#    print('Huffman Code =', output,'for n=',n)
    ...
    print('First 10 Huffman Codes =',output[0:10],'Average Length =', sum(L)/n,'for n =',n)
    listL.append(sum(L)/n)
